...
fn safe_get<T: Copy>(slice: &[T], index: usize) -> Option<T> {
    if slice.len() > index {Some(slice[index])}
                      else {None}
}
...
fn transpose_blocks (data: &[u8], blocksize: usize) -> Vec<Vec<u8>> {
    ...
    for k in 0..blocksize {
        let block: Vec<u8> = data.chunks(blocksize).filter_map(|s| safe_get(s, k)).collect();
        out.push(block)
    }
    out
}
...
fn main() {
    ...
    println!("{}", str::from_utf8(&crypto::repeating_key_xor(&data, &key)).unwrap());
