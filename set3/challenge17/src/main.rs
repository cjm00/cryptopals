...
impl CryptoServer {
    ...
    fn decrypt_message(&self,
                       message: &[u8],
                       iv: &[u8])
                       -> Result<UnencryptedBytes, block::BlockEncryptionError> {
        block::aes_cbc_decrypt(message, &self.key, iv)
    }
}
...
fn padding_oracle_attack(server: CryptoServer) -> String {
    ...
    while known.len() < initial.len() {
        ...
        let known: Vec<u8> = known.chunks(KEYSIZE).flat_map(|s| s.iter().cloned().rev()).collect();
        println!("{}\n {} {}",
                 str::from_utf8(&known).unwrap(),
                 known.len(),
                 initial.len());
        unreachable!();
    }
...
fn main() {
    let l = pop_random_element(&mut load_data());
    let server = CryptoServer::new(l);
